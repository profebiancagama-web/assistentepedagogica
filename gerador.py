...
SUPABASE_KEY = "import streamlit as st
from docx import Document
from docx.shared import Pt, RGBColor
from google import genai
from google.genai import types
...
# 🔑 SE VOCÊ TIVER UMA CHAVE ATUALIZADA DO GOOGLE (AIzaSy...), COLE ELA AQUI DENTRO DAS ASPAS:
...
# --- CONFIGURAÇÃO NATIVA DE CHAT COM TRATAMENTO DE ERRO DE AUTENTICAÇÃO ---
    ...
    if st.button("✨ GERAR PLANO ANUAL", key="b1"):
        with st.spinner("Processando..."):
            try:
                ...
                st.error(f"Erro de Conexão com a Chave Gemini: Verifique suas credenciais de IA. Técnico: {e}")
...
# --- 💬 ABA DE CHAT COM PROTEÇÃO ADICIONAL DE AUTENTICAÇÃO ---
with aba_chat:
    ...
    if prompt := st.chat_input("Digite sua mensagem para a B.IA..."):
        ...
        with St.spinner("B.IA está pensando..."):
            if st.session_state.objeto_chat is None:
                # Tentativa de reconstrução caso a chave mude sem reiniciar o app
                try:
                    st.session_state.objeto_chat = st.session_state.client.chats.create(model="gemini-2.5-flash")
                except: pass
            ...
            if st.session_state.objeto_chat is not None:
                try:
                    ...
                    st.error(f"⚠️ Erro de Autenticação na API: Sua chave de acesso do Gemini expirou ou está inválida no código. Técnico: {e}")
            else:
                st.error("⚠️ Não foi possível iniciar o chat. Por favor, atualize a variável 'MINHA_CHAVE' no código do GitHub com uma credencial válida do Google AI Studio.")
